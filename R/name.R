...
gen.lastname <- function(country='uk', race =NA){
  if (tolower(country)=='uk'){
    lastname <- read.csv(file = "data/lastname_uk.csv", header = TRUE, sep=",", stringsAsFactors = FALSE)
    outputname = sample(lastname[,1],size=1, replace=TRUE, prob =lastname[,2])
  }
  else{
    ...
    if (!is.na(race)){
      ...
      outputname = sample(lastname[lastname$race ==race,1],
                          size=1, replace=TRUE,
                          prob =lastname[lastname$race ==race,2])
    }
    else {
      outputname = sample(lastname[,1],size=1, replace=TRUE, prob =lastname[,2])
    }
...
get_transformation_name_variant<- function (string){
  do_name_replacement<- function(s){
    outputname = s
    firstname_variant <- read.csv(file = "data/firstname_uk_variant.csv", header = TRUE, sep=",", stringsAsFactors = FALSE)
    lastname_variant <- read.csv(file = "data/lastname_uk_variant.csv", header = TRUE, sep=",", stringsAsFactors = FALSE)
