...
get_transformation_del<- function (string){
  if(string!='' && nchar(string)>1){
    ...
    # if we have statatiscs that letter 3 has a higher prob of being deleted than letter 2 than letter 5 than....
    # prob = c(0.1, 0.3, 0.5, 0.1, 0.2)
    # del_position = sample(nchar(string),size = 1, replace = TRUE, prob = prob)

    # otherwise, treat them as equal prob
    del_position = sample(nchar(string),size = 1, replace = TRUE)
    changestr = paste0(substr(string, 1, del_position-1),substr(string, del_position+1,nchar(string)))
    newstr = paste0(changestr,',', substr(string,del_position,del_position), '>del>',del_position)
  }
  else{
...
get_transformation_trans_char<- function (string){
  if(string!='' && nchar(string)>1){
    ...
    newstr = paste0(changestr,',', substr(string,trans_position,trans_position),
                    substr(string,trans_position+1,trans_position+1), '>trans>',trans_position,trans_position+1)
  }
  else{
    newstr = paste0(string, ',string either empty or shorter than 2 characters')
  }
...
get_transformation_insert<- function (string){
  if(string!=''){
    ...
    changestr = paste0(substr(string, 1, insert_position),insertchar, substr(string, insert_position+1,nchar(string)))
...
# get_transformation_insert<- function (string, type= 'char'){
#   if(string!=''){
#     if (type=='char'){
...
#       dict = '`~!@£$%^&*()_+=-[];,./{}|:"<>?'
#     } else {
#       newstr = paste0(string, ',type should be either character,digit,space or symbol')
#       return(newstr)
#     }
#     insertchar = sample(strsplit(dict,'')[[1]],1)
#     insert_position = sample(nchar(string),size = 1, replace = TRUE)
